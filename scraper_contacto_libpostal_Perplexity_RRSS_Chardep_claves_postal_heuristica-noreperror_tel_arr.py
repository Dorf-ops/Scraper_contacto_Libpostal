...
from postal.parser import parse_address  # <--- Usamos la librería postal
...
def extraer_telefonos(texto):
    # Solo teléfonos españoles válidos: 9 cifras, con o sin prefijo, separadores permitidos
    posibles = re.findall(
...
def llamar_libpostal(texto):
    try:
        ...
        if parsed and any(label in dict(parsed) for label in ["road", "city", "postcode"]):
